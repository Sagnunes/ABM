...
@endsection
@section('content')
    @include('forms.noticias.modal')
    @include('forms.noticias.alert-modal-delete')
                        ...
                        @foreach($results as $item)
                            <tr>
                                ...
                                <td>
                                    ...
                                    @if (Auth::user()->acessos->where('modulo_id',17)->where('outros','>',3)->first())
                                    <a href="{{route('noticias.edit',['id'=>$item->id])}}" class="m-portlet__nav-link btn m-btn m-btn--hover-accent m-btn--icon m-btn--icon-only m-btn--pill" title="Editar detalhes">
                                            <i class="la la-edit"></i>
                                        </a>
                                    @endif
                                    @if (Auth::user()->acessos->where('modulo_id',17)->where('outros','=',4)->first())
